...
int main(int argc, char** argv) {
    STRPTR template = (STRPTR)"FORMAT/A,COLOR/K,ARG1/K,ARG2/K,ARG3/K";
    struct {
        char* format;
        char* color;
        char* arg1;
    ...
    if (!rdargs) {
        PrintFault(IoErr(), (STRPTR)"shprintf");
        return EXIT_FAILURE;
    }

    const char* color = getColor(args.color);
    shellPrintf(color, args.format, args.arg1, args.arg2, args.arg3);
    printf("\n");
